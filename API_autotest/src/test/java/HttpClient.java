...
public class HttpClient {
    ...
    public static User sendPOST(CloseableHttpClient srv, String postParam) throws IOException {
        ...
        ResponseHandler<User> responseHandler = new ResponseHandler<User>() {
            @Override
            public User handleResponse(final HttpResponse response) throws IOException {
    ...
    public static ResponseBody sendPUT(CloseableHttpClient srv, String putParam) throws IOException {
        ...
        ResponseHandler<ResponseBody> responseHandler = new ResponseHandler<ResponseBody>() {
            @Override
            public ResponseBody handleResponse(final HttpResponse response) throws IOException {
                int status = response.getStatusLine().getStatusCode();
                if (status >= 200 && status < 300) {
                    HttpEntity entity = response.getEntity();
                    if (entity != null) {
